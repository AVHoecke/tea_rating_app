 ...
 */
	public function testPath() {
		...
		$this->assertEquals($expected, Plugin::path('TestPlugin'));

		$expected = CAKE . 'Test' . DS . 'test_app' . DS . 'Plugin' . DS . 'TestPluginTwo' . DS;
		$this->assertEquals($expected, Plugin::path('TestPluginTwo'));
	}
...
/**
 * Tests that Plugin::loadAll() will load all plugins in the configured folder with defaults
 ...
 */
	public function testLoadAllWithDefaultsAndOverrideComplex() {
		Plugin::loadAll(array(array('bootstrap' => true), 'TestPlugin' => array('routes' => true, 'bootstrap' => false)));
		Plugin::routes();
