 ...
 */
	protected function _verify($config) {
		...
		$this->hr();
		$this->out(__d('cake_console', "Name:         {0}", $name));
		$this->out(__d('cake_console', "Datasource:   {0}", $datasource));
		$this->out(__d('cake_console', "Persistent:   {0}", $persistent));
		$this->out(__d('cake_console', "Host:         {0}", $host));

		if ($port) {
			$this->out(__d('cake_console', "Port:         {0}", $port));
		}

		$this->out(__d('cake_console', "User:         {0}", $login));
		$this->out(__d('cake_console', "Pass:         {0}", str_repeat('*', strlen($password))));
		$this->out(__d('cake_console', "Database:     {0}", $database));

		if ($prefix) {
			$this->out(__d('cake_console', "Table prefix: {0}", $prefix));
		}

		if ($schema) {
			$this->out(__d('cake_console', "Schema:       {0}", $schema));
		}

		if ($encoding) {
			$this->out(__d('cake_console', "Encoding:     {0}", $encoding));
		}
 ...
 */
	public function bake($configs) {
		if (!is_dir($this->path)) {
			$this->err(__d('cake_console', '{0} not found', $this->path));
			return false;
		}
