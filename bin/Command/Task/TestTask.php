 ...
 */
	protected function _interactive($type = null) {
		...
		$this->out(__d('cake_console', 'Path: {0}', $this->getPath()));
		$this->hr();

		if ($type) {
			$type = Inflector::camelize($type);
			if (!isset($this->classTypes[$type])) {
				$this->error(__d('cake_console', 'Incorrect type provided. Please choose one of {0}', implode(', ', array_keys($this->classTypes))));
			}
		} else {
 ...
 */
	public function bake($type, $className) {
		...
		$this->out("\n" . __d('cake_console', 'Baking test case for {0} {1} ...', $className, $type), 1, Shell::QUIET);
 ...
 */
	public function getClassName($objectType) {
		...
		$this->out(__d('cake_console', 'Choose a {0} class', $objectType));
		$keys = array();
 ...
 * @return string
 * @throws \Exception When invalid object types are requested.
 */
	public function mapType($type, $plugin) {
		$type = ucfirst($type);
		if (empty($this->classTypes[$type])) {
			throw new \Exception(__d('cake_dev', 'Invalid object type.'));
		}
 ...
 * @throws \Exception on invalid types.
 */
	public function getBaseType($type) {
		if (empty($this->baseTypes[$type])) {
			throw new \Exception(__d('cake_dev', 'Invalid type name'));
		}
 ...
 */
	public function generateConstructor($type, $fullClassName, $plugin) {
		...
		if ($type === 'component') {
			$pre = "\$Collection = new ComponentRegistry();\n";
 ...
 */
	public function generateUses($type, $realType, $className) {
		...
		$type = strtolower($type);
		if ($type === 'component') {
			$uses[] = array('ComponentRegistry', 'Controller');
