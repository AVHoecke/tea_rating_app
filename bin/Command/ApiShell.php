 ...
 */
	public function main() {
		...
		if (in_array($class, $objects)) {
			...
			$this->error(__d('cake_console', '{0} not found', $class));
		}
		...
		if (!empty($parsed)) {
			if (isset($this->params['method'])) {
				if (!isset($parsed[$this->params['method']])) {
					$this->err(__d('cake_console', '{0}::{1}() could not be found', $class, $this->params['method']));
					return $this->_stop();
				}
 ...
 */
	public function help() {
		...
		if (!isset($this->args[1])) {
			...
			$this->out(__d('cake_console', 'Command {0} not found', $this->args[1]));
		}
	}
	...
	protected function _parseClass($path, $class) {
		...
		if (!class_exists($class)) {
			if (!include_once $path) {
				$this->err(__d('cake_console', '{0} could not be found', $path));
			}
		}
