		...
		foreach ($this->_args as $i => $arg) {
			if ($arg->isRequired() && !isset($args[$i]) && empty($params['help'])) {
				throw new ConsoleException(
					__d('cake_console', 'Missing required arguments. {0} is required.', $arg->name())
				);
			}
		}
 ...
 */
	protected function _parseShortOption($option, $params) {
		...
		if (!isset($this->_shortOptions[$key])) {
			throw new ConsoleException(__d('cake_console', 'Unknown short option `{0}`', $key));
		}
 ...
 */
	protected function _parseOption($name, $params) {
		if (!isset($this->_options[$name])) {
			throw new ConsoleException(__d('cake_console', 'Unknown option `{0}`', $name));
		}
