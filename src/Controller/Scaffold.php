	...
	protected function _scaffoldView(Request $request) {
		if ($this->controller->beforeScaffold('view')) {
			...
			if (!$this->ScaffoldModel->exists()) {
				throw new NotFoundException(__d('cake', 'Invalid {0}', Inflector::humanize($this->modelKey)));
			}
			...
			if (!empty($request->data)) {
				...
				if ($this->ScaffoldModel->save($request->data)) {
					if ($this->controller->afterScaffoldSave($action)) {
						$message = __d('cake', 'The %1$s has been %2$s',
 ...
 */
	protected function _scaffoldDelete(Request $request) {
		if ($this->controller->beforeScaffold('delete')) {
			...
			if (!$this->ScaffoldModel->exists()) {
				throw new NotFoundException(__d('cake', 'Invalid {0}', Inflector::humanize($this->modelClass)));
			}
			if ($this->ScaffoldModel->delete()) {
				$message = __d('cake', 'The %1$s with id: %2$s has been deleted.', Inflector::humanize($this->modelClass), $id);
				return $this->_sendMessage($message, 'success');
			}
			$message = __d('cake', 'There was an error deleting the %1$s with id: %2$s',
 ...
 */
	protected function _scaffold(Request $request) {
		$db = ConnectionManager::get($this->ScaffoldModel->useDbConfig);
