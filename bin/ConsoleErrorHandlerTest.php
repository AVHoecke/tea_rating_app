...
/**
 * test a non \Exception exception.
 *
 * @return void
 */
	public function testNon\Exceptions() {
 ...
 */
	public function testError404Exception() {
		$exception = new NotFoundException('dont
		ConsoleErrorHandler::$stderr->expects($this->once())->method('write')
			->with($this->stringContains('dont
		$this->Error->expects($this->once())
			->method('_stop')
			->with(404);
 ...
 */
	public function testError500Exception() {
		$exception = new InternalErrorException('dont
		ConsoleErrorHandler::$stderr->expects($this->once())->method('write')
			->with($this->stringContains('dont
		$this->Error->expects($this->once())
			->method('_stop')
