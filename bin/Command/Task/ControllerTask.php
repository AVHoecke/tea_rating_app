	...
	public function main() {
		...
		if (isset($this->args[0])) {
			...
			if (!empty($this->params['admin'])) {
				...
				if ($admin) {
					$this->out(__d('cake_console', 'Adding {0} methods', $admin));
 ...
 */
	protected function _interactive() {
		...
		$this->hr();
		$this->out(__d('cake_console', 'Baking {0}Controller', $controllerName));
		$this->hr();
		...
		if (file_exists($this->path . $controllerName . 'Controller.php')) {
			$question[] = __d('cake_console', "Warning: Choosing no will overwrite the {0}Controller.", $controllerName);
		}
 ...
 */
	public function bakeActions($controllerName, $admin = null, $wannaUseSession = true) {
		...
		/* TODO: App::uses($modelImport, $plugin . 'Model'); */
 ...
 */
	public function bake($controllerName, $actions = '', $helpers = null, $components = null) {
		$this->out("\n" . __d('cake_console', 'Baking controller class for {0}...', $controllerName), 1, Shell::QUIET);
