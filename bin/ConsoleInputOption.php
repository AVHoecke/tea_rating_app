	...
	public function __construct($name, $short = null, $help = '', $boolean = false, $default = '', $choices = array()) {
		...
		if (strlen($this->_short) > 1) {
			throw new ConsoleException(
				__d('cake_console', 'Short option "{0}" is invalid, short options must be one letter.', $this->_short)
			);
		}
	}
 ...
 */
	public function help($width = 0) {
		...
		if (!empty($this->_default) && $this->_default !== true) {
			$default = __d('cake_console', ' <comment>(default: {0})</comment>', $this->_default);
		}
		if (!empty($this->_choices)) {
			$default .= __d('cake_console', ' <comment>(choices: {0})</comment>', implode('|', $this->_choices));
		}
 ...
 */
	public function validChoice($value) {
		...
		if (!in_array($value, $this->_choices)) {
			throw new ConsoleException(
				__d('cake_console', '"{0}" is not a valid value for --{1}. Please use one of "{2}"',
