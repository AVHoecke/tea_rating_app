 ...
 */
	public function setup(Model $Model, $config = array()) {
		$db = ConnectionManager::get($Model->useDbConfig);
		if (!$db->connected) {
			trigger_error(
				__d('cake_dev', 'Datasource {0} for TranslateBehavior of model {1} is not connected', $Model->useDbConfig, $Model->alias),
				E_USER_ERROR
			);
			return false;
		}
 ...
 * @return bool
 * @throws \Exception when attempting to bind a translating called name. This is not allowed
			...
			if ($association === null) {
				...
				foreach (array('hasOne', 'hasMany', 'belongsTo', 'hasAndBelongsToMany') as $type) {
					if (isset($Model->{$type}[$association]) || isset($Model->__backAssociation[$type][$association])) {
						trigger_error(
							__d('cake_dev', 'Association {0} is already bound to model {1}', $association, $Model->alias),
							E_USER_ERROR
						);
						return false;
					}
