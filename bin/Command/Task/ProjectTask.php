 ...
 */
	public function main() {
		...
		while (!$response && is_dir($project) === true && file_exists($project . 'Config' . 'core.php')) {
			$prompt = __d('cake_console', '<warning>A project already exists in this location:</warning> {0} Overwrite?', $project);
		...
		if ($this->bake($project)) {
			...
			if ($this->securitySalt($path) === true) {
				...
				$this->err(__d('cake_console', 'Unable to generate random hash for \'Security.salt\', you should change it in {0}', CONFIG . 'core.php'));
				$success = false;
			}
			...
			if ($this->securityCipherSeed($path) === true) {
				...
				$this->err(__d('cake_console', 'Unable to generate random seed for \'Security.cipherSeed\', you should change it in {0}', CONFIG . 'core.php'));
				$success = false;
			}
			...
			if ($success) {
				$this->out(__d('cake_console', ' * CAKE_CORE_INCLUDE_PATH set to {0} in {1}', CAKE_CORE_INCLUDE_PATH, 'webroot/index.php'));
				$this->out(__d('cake_console', ' * CAKE_CORE_INCLUDE_PATH set to {0} in {1}', CAKE_CORE_INCLUDE_PATH, 'webroot/test.php'));
			} else {
				$this->err(__d('cake_console', 'Unable to set CAKE_CORE_INCLUDE_PATH, you should change it in {0}', $path . 'webroot' . DS . 'index.php'));
				$success = false;
			}
			if ($success && $hardCode) {
			...
			if (!$Folder->chmod($path . 'tmp', 0777)) {
				$this->err(__d('cake_console', 'Could not set permissions on {0}', $path . DS . 'tmp'));
		...
		switch (strtolower($looksGood)) {
			case 'y':
				...
				if ($Folder->copy(array('to' => $path, 'skip' => $skip))) {
					$this->hr();
					$this->out(__d('cake_console', '<success>Created:</success> {0} in {1}', $app, $path));
					$this->hr();
				} else {
					$this->err(__d('cake_console', "<error>Could not create</error> '{0}' properly.", $app));
					return false;
				}
 ...
 */
	public function getPrefix() {
		...
		if ($this->interactive) {
			$this->hr();
			$this->out(__d('cake_console', 'You need to enable {0} in {1} to use prefix routing.',
					'Configure::write(\'Routing.prefixes\', array(\'admin\'))',
					'/app/Config/core.php'));
			$this->out(__d('cake_console', 'What would you like the prefix route to be?'));
			$this->out(__d('cake_console', 'Example: {0}', 'www.example.com/admin/controller'));
			while (!$admin) {
				$admin = $this->in(__d('cake_console', 'Enter a routing prefix:'), null, 'admin');
			}
			if ($this->cakeAdmin($admin) !== true) {
				$this->out(__d('cake_console', '<error>Unable to write to</error> {0}.', '/app/Config/core.php'));
				$this->out(__d('cake_console', 'You need to enable {0} in {1} to use prefix routing.',
					'Configure::write(\'Routing.prefixes\', array(\'admin\'))',
					'/app/Config/core.php'));
				return $this->_stop();
