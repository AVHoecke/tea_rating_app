 ...
 */
	public function all() {
		...
		foreach ($this->_tables as $table) {
			...
			$this->out(__d('cake_console', 'Baking {0}', $modelClass));
 ...
 */
	protected function _interactive() {
		...
		if (!in_array($useTable, $this->_tables)) {
			$prompt = __d('cake_console', "The table {0} doesn't exist or could not be automatically detected\ncontinue anyway?", $useTable);
		...
		$this->hr();
		$this->out(__d('cake_console', "Name:       {0}", $currentModelName));

		if ($this->connection !== 'default') {
			$this->out(__d('cake_console', "DB Config:  {0}", $this->connection));
		}
		if ($fullTableName !== Inflector::tableize($currentModelName)) {
			$this->out(__d('cake_console', 'DB Table:   {0}', $fullTableName));
		}
		if ($primaryKey !== 'id') {
			$this->out(__d('cake_console', 'Primary Key: {0}', $primaryKey));
		}
		if (!empty($validate)) {
			$this->out(__d('cake_console', 'Validation: {0}', print_r($validate, true)));
		}
 ...
 */
	public function fieldValidation($fieldName, $metaData, $primaryKey = 'id') {
		...
		while ($anotherValidator === 'y') {
			if ($this->interactive) {
				$this->out();
				$this->out(__d('cake_console', 'Field: <info>{0}</info>', $fieldName));
				$this->out(__d('cake_console', 'Type: <info>{0}</info>', $metaData['type']));
				$this->hr();
				...
				$this->out($optionText);
				$this->out(__d('cake_console', "{0} - Do not do any validation on this field.", $defaultChoice));
				$this->hr();
			}
 ...
 */
	public function doMoreAssociations(Model $model, $associations) {
		...
		while (strtolower($wannaDoMoreAssoc) === 'y') {
			...
			$className = $this->in(__d('cake_console', 'What className will {0} use?', $alias), null, $alias);
 ...
 */
	public function bake($name, $data = array()) {
		...
		$this->out("\n" . __d('cake_console', 'Baking model class for {0}...', $name), 1, Shell::QUIET);
 ...
 */
	public function getTable($modelName, $useDbConfig = null) {
		...
		if ($this->interactive === true) {
			...
			$db = ConnectionManager::get($useDbConfig);
			$fullTableName = $db->fullTableName($useTable, false);
			$tableIsGood = false;
			if (array_search($useTable, $this->_tables) === false) {
				$this->out();
				$this->out(__d('cake_console', "Given your model named '{0}',\nCake would expect a database table named '{1}'", $modelName, $fullTableName));
				$tableIsGood = $this->in(__d('cake_console', 'Do you want to			}
