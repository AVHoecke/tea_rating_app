 ...
 * @throws \Exception when binding to a model that doesn't exist.
 */
	public function node($ref = null) {
		...
		if (empty($ref)) {
			...
			if (empty($model)) {
				throw new \Exception('cake_dev', "Model class '%s' not found in AclNode::node() when trying to bind %s object", $type, $this->alias);
			}
