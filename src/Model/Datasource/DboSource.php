 ...
 * @return mixed
 * @throws \Exception when results cannot be created.
 */
	public function queryAssociation(Model $Model, Model $LinkModel, $type, $association, $assocData, &$queryData, $external, &$resultSet, $recursive, $stack) {
		...
		if (!is_array($resultSet)) {
			throw new \Exception(__d('cake_dev', 'Error in Model {0}', get_class($Model)));
		}
 ...
 */
	public function buildColumn($column) {
		...
		if (!isset($this->columns[$type]) && substr($type, 0, 4) !== 'enum') {
			trigger_error(__d('cake_dev', 'Column type {0} does not exist', $type), E_USER_WARNING);
			return null;
		}
