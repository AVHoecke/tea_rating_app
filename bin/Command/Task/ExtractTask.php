 ...
 */
	public function main() {
		...
		if (!$this->_isPathUsable($this->_output)) {
			$this->err(__d('cake_console', 'The output directory {0} was not found or writable.', $this->_output));
			return $this->_stop();
		}
 ...
 */
	protected function _extractTokens() {
		foreach ($this->_files as $file) {
			$this->_file = $file;
			$this->out(__d('cake_console', 'Processing {0}...', $file), 1, Shell::VERBOSE);
		...
		foreach ($this->_storage as $category => $domains) {
			foreach ($domains as $domain => $sentences) {
				...
				while ($overwriteAll === false && $File->exists() && strtoupper($response) !== 'Y') {
					$this->out();
					$response = $this->in(
						__d('cake_console', 'Error: {0} already exists in this location. Overwrite? [Y]es, [N]o, [A]ll', $filename),
						array('y', 'n', 'a'),
						'y'
					);
 ...
 */
	protected function _markerError($file, $line, $marker, $count) {
		$this->err(__d('cake_console', "Invalid marker content in {0}:{1}\n* {2}(", $file, $line, $marker));
		$count += 2;
		$tokenCount = count($this->_tokens);
		$parenthesis = 1;
