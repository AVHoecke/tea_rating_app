 ...
 */
	protected function _welcome() {
		...
		$this->out(__d('cake_console', 'DocumentRoot: {0}', $this->_documentRoot));
		$this->hr();
	}
 ...
 */
	public function main() {
		if (version_compare(PHP_VERSION, '5.4.0') < 0) {
			$this->out(__d('cake_console', '<warning>This command is available on {0} or above</warning>', 'PHP5.4'));
			return;
		}
		...
		$this->out(__d('cake_console', 'built-in server is running in http://{0}{1}/', $this->_host, $port));
		system($command);
	}
