	...
	public function main() {
		if (isset($this->args[0])) {
			...
			if (is_dir($pluginPath)) {
				$this->out(__d('cake_console', 'Plugin: {0} already exists, no action taken', $plugin));
				$this->out(__d('cake_console', 'Path: {0}', $pluginPath));
				return false;
			}
 ...
 */
	protected function _interactive($plugin = null) {
		...
		if (!$this->bake($plugin)) {
			$this->error(__d('cake_console', "An error occurred trying to bake: {0} in {1}", $plugin, $this->path . $plugin));
		}
	}
 ...
 */
	public function bake($plugin) {
		...
		$this->hr();
		$this->out(__d('cake_console', "<info>Plugin Name:</info> {0}", $plugin));
		$this->out(__d('cake_console', "<info>Plugin Directory:</info> {0}", $this->path . $plugin));
		$this->hr();
		...
		if (strtolower($looksGood) === 'y') {
			...
			$out = "<?php\n\n";
			$out .= "			$out .= "class {$plugin}AppController extends AppController {\n\n";
			$out .= "}\n";
			...
			$out = "<?php\n\n";
			$out .= "			$out .= "class {$plugin}AppModel extends AppModel {\n\n";
			$out .= "}\n";
			...
			$this->hr();
			$this->out(__d('cake_console', '<success>Created:</success> {0} in {1}', $plugin, $this->path . $plugin), 2);
		}
 ...
 */
	protected function _modifyBootstrap($plugin) {
		...
		if (!preg_match("@\n\s*Plugin::loadAll@", $contents)) {
			...
			$this->out('');
			$this->out(__d('cake_dev', '{0} modified', $this->bootstrap));
		}
	}
