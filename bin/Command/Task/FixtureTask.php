...
use as conditions\nExample: WHERE 1=1");
use record importing for this fixture?'), array('y', 'n'), 'n');
 ...
 */
	public function importOptions($modelName) {
		...
		if (!empty($this->params['records'])) {
			...
			$doRecords = $this->in(__d('cake_console', 'Would you like to			if ($doRecords === 'y') {
				$options['records'] = true;
			}
		}
		if (!isset($options['records']) && $this->interactive) {
			$prompt = __d('cake_console', "Would you like to build this fixture with data from {0}'s table?", $modelName);
 ...
 */
	public function generateFixtureFile($model, $otherVars) {
		...
		$this->out("\n" . __d('cake_console', 'Baking test fixture for {0}...', $model), 1, Shell::QUIET);
 ...
 */
	protected function _getRecordsFromTable($modelName, $useTable = null) {
		...
		if ($this->interactive) {
			$condition = null;
			$prompt = __d('cake_console', "Please provide a SQL fragment to			while (!$condition) {
