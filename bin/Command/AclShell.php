...
use DbAcl before attempting to use this script') . "\n";
 ...
 */
	public function startup() {
		...
		if (!in_array($class, array('DbAcl', 'DB_ACL')) && !is_subclass_of($class, 'DbAcl')) {
			...
			$out .= __d('cake_console', 'Please change your core config to reflect your decision to			$out .= "--------------------------------------------------\n";
			$out .= __d('cake_console', 'Current ACL Classname: {0}', $class) . "\n";
			$out .= "--------------------------------------------------\n";
			$this->err($out);
			return $this->_stop();
		}
 ...
 */
	public function create() {
		...
		if ($this->Acl->{$class}->save($data)) {
			$this->out(__d('cake_console', "<success>New {0}</success> '{1}' created.", $class, $this->args[2]), 2);
		} else {
			$this->err(__d('cake_console', "There was a problem creating a new {0} '{1}'.", $class, $this->args[2]));
		}
	}
 ...
 */
	public function delete() {
		...
		if ($this->Acl->{$class}->find('all', array('conditions' => $identifier))) {
			...
			$this->out(__d('cake_console', '<success>{0} deleted.</success>', $class), 2);
		} else {
 ...
 */
	public function setParent() {
		...
		if (!$this->Acl->{$class}->save($data)) {
			...
			$this->out(__d('cake_console', 'Node parent set to {0}', $this->args[2]) . "\n");
		}
	}
 ...
 */
	public function getPath() {
		...
		if (empty($nodes)) {
			$this->error(
				__d('cake_console', "Supplied Node '{0}' not found", $this->args[1]),
 ...
 */
	public function check() {
		...
		if ($this->Acl->check($aro, $aco, $action)) {
			$this->out(__d('cake_console', '{0} is <success>allowed</success>.', $aroName));
		} else {
			$this->out(__d('cake_console', '{0} is <error>not allowed</error>.', $aroName));
		}
	}
 ...
 */
	public function view() {
		...
		if (empty($nodes)) {
			if (isset($this->args[1])) {
				$this->error(__d('cake_console', '{0} not found', $this->args[1]), __d('cake_console', 'No tree returned.'));
			} elseif (isset($this->args[0])) {
				$this->error(__d('cake_console', '{0} not found', $this->args[0]), __d('cake_console', 'No tree returned.'));
			}
		}
		$this->out($class . ' tree:');
		$this->hr();
 ...
 */
	protected function _getNodeId($class, $identifier) {
		...
		if (empty($node)) {
			...
			$this->error(__d('cake_console', 'Could not find node using reference "{0}"', $identifier));
			return null;
		}
