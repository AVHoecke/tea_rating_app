...
use App\Model\BehaviorCollection;
use App\Model\ConnectionManager;
use App\Model\ModelBehavior;
use App\Model\ModelValidator;
 ...
 */
class Model extends CakeObject implements EventListener {
 ...
 */
	public function getDataSource() {
		...
		return ConnectionManager::get($this->useDbConfig);
	}
 ...
 */
	public function joinModel($assoc, $keys = array()) {
		...
		trigger_error(
			__d('cake_dev', 'Invalid join model settings in {0}. The association parameter has the wrong type, expecting a string or array, but was passed type: {1}', $this->alias, gettype($assoc)),
			E_USER_WARNING
		);
	}
