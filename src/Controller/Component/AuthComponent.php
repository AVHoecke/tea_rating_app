	...
	public function constructAuthorize() {
		...
		foreach ($config as $class => $settings) {
			...
			if (!class_exists($className)) {
				throw new \Exception(__d('cake_dev', 'Authorization adapter "{0}" was not found.', $class));
			}
			if (!method_exists($className, 'authorize')) {
				throw new \Exception(__d('cake_dev', 'Authorization objects must implement an {0} method.', 'authorize()'));
			}
 ...
 */
	public function constructAuthenticate() {
		...
		foreach ($config as $class => $settings) {
			...
			/* TODO: App::uses($className, $plugin . 'Controller/Component/Auth'); */
			if (!class_exists($className)) {
				throw new \Exception(__d('cake_dev', 'Authentication adapter "{0}" was not found.', $class));
			}
			if (!method_exists($className, 'authenticate')) {
				throw new \Exception(__d('cake_dev', 'Authentication objects must implement an {0} method.', 'authenticate()'));
			}
			$settings = array_merge($global, (array)$settings);
			$auth = new $className($this->_registry, $settings);
			$this->_registry->getController()->getEventManager()->attach($auth);
