 ...
 */
	public function read($options = array()) {
		...
		$db = ConnectionManager::get($options['connection']);
		...
		foreach ($fields as $name => $value) {
			...
			if (substr($value['type'], 0, 4) !== 'enum') {
				if (!isset($db->columns[$value['type']])) {
					trigger_error(__d('cake_dev', 'Schema generation error: invalid column type {0} for {1}.{2} does not exist in DBO', $value['type'], $Obj->name, $name), E_USER_NOTICE);
					continue;
				} else {
