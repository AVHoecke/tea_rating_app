...
use Cake\View\View;
use `url` directly instead.', E_USER_DEPRECATED);
 ...
 */
	public function create($model = null, $options = array()) {
		...
		if (isset($options['action'])) {
			trigger_error('Using key `action` is deprecated,		}
 ...
 */
	public function error($field, $text = null, $options = array()) {
		...
		if (is_array($error)) {
			foreach ($error as &$e) {
				if (is_numeric($e)) {
					$e = __d('cake', 'Error in field {0}', Inflector::humanize($this->field()));
				}
			}
		}
		if ($options['escape']) {
 ...
 * @throws \Exception When there are no params for the method call.
 */
	public function __call($method, $params) {
		$options = array();
		if (empty($params)) {
			throw new \Exception(__d('cake_dev', 'Missing field name for FormHelper::{0}', $method));
		}
		if (isset($params[1])) {
			$options = $params[1];
		}
