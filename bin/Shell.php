 ...
 */
	protected function _welcome() {
		$this->out();
		$this->out(__d('cake_console', '<info>Welcome to CakePHP {0} Console</info>', 'v' . Configure::version()));
		$this->hr();
		$this->out(__d('cake_console', 'App : {0}', APP_DIR));
		$this->out(__d('cake_console', 'Path: {0}', APP));
		$this->hr();
	}
 ...
 */
	public function loadTasks() {
		...
		$this->_taskMap = TaskRegistry::normalizeObjectArray((array)$this->tasks);
 ...
 */
	public function runCommand($command, $argv) {
		...
		try {
			...
			$this->err(__d('cake_console', '<error>Error:</error> {0}', $e->getMessage()));
 ...
 */
	public function error($title, $message = null) {
		$this->err(__d('cake_console', '<error>Error:</error> {0}', $title));
 ...
 */
	public function createFile($path, $contents) {
		...
		if (is_file($path) && empty($this->params['force']) && $this->interactive === true) {
			$this->out(__d('cake_console', '<warning>File `{0}` exists</warning>', $path));
			...
			if (strtolower($key) === 'q') {
				...
				$this->out(__d('cake_console', 'Skip `{0}`', $path), 2);
				return false;
			}
		} else {
			$this->out(__d('cake_console', 'Creating file {0}', $path));
		}
		...
		if ($File->exists() && $File->writable()) {
			...
			$File->write($data);
			$this->out(__d('cake_console', '<success>Wrote</success> `{0}`', $path));
			return true;
		}

		$this->err(__d('cake_console', '<error>Could not write to `{0}`</error>.', $path), 2);
		return false;
	}
 ...
 */
	protected function _checkUnitTest() {
		...
		if ($result) {
			$this->out();
			$this->out(__d('cake_console', 'You can download PHPUnit from {0}', 'http://phpunit.de'));
		}
		return $result;
	}
