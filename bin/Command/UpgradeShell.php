 ...
 */
	public function all() {
		foreach ($this->OptionParser->subcommands() as $command) {
			...
			$this->out(__d('cake_console', 'Running {0}', $name));
			$this->$name();
		}
	}
 ...
 */
	public function tests() {
		...
		if (!empty($this->params['plugin'])) {
			$this->_paths = array(Plugin::path($this->params['plugin']) . 'tests' . DS);
		}
		$patterns = array(
			array(
				'*TestCase extends TestCase to *Test extends TestCase',
				'/([a-zA-Z]*Test)Case extends TestCase/',
				'\1 extends TestCase'
			),
		);
 ...
 */
	public function locations() {
		...
		if (!empty($this->params['plugin'])) {
			chdir(Plugin::path($this->params['plugin']));
		}
		...
		if (is_dir('plugins')) {
			...
			foreach ($plugins as $plugin) {
				...
				$this->out(__d('cake_console', 'Upgrading locations for plugin {0}', $plugin));
				$this->locations();
			}
			$this->_files = array();
			chdir($cwd);
		...
		foreach ($moves as $old => $new) {
			if (is_dir($old)) {
				$this->out(__d('cake_console', 'Moving {0} to {1}', $old, $new));
				if (!$this->params['dry-run']) {
					if ($this->params['git']) {
		...
		$sourceDirs = array(
			...
			'Model',
			'tests',
			'Test' => array('regex' => '@class (\S*Test) extends TestCase@'),
			'views',
			'View',
			'vendors/shells',
		);
 ...
 */
	public function helpers() {
		...
		if (!empty($this->params['plugin'])) {
			$this->_paths = array(Plugin::path($this->params['plugin']) . 'views' . DS);
		}
		...
		foreach ($plugins as $plugin) {
			...
			$pluginHelpers = array_merge(
				$pluginHelpers,
				App::objects('helper', Plugin::path($plugin) . DS . 'views' . DS . 'helpers' . DS, false)
			);
		}
...
/**
 * Update the properties moved to Request.
 *
 * @return void
 */
 ...
 */
	protected function _movePhpFiles($path, $options) {
		...
		foreach ($this->_files as &$file) {
			...
			$this->out(__d('cake_console', 'Moving {0} to {1}', $file, $new), 1, Shell::VERBOSE);
			if (!$this->params['dry-run']) {
				if ($this->params['git']) {
 ...
 */
	protected function _filesRegexpUpdate($patterns) {
		...
		foreach ($this->_files as $file) {
			$this->out(__d('cake_console', 'Updating {0}...', $file), 1, Shell::VERBOSE);
 ...
 */
	protected function _updateFile($file, $patterns) {
		...
		foreach ($patterns as $pattern) {
			$this->out(__d('cake_console', ' * Updating {0}', $pattern[0]), 1, Shell::VERBOSE);
			$contents = preg_replace($pattern[1], $pattern[2], $contents);
		}

		$this->out(__d('cake_console', 'Done updating {0}', $file), 1);
		if (!$this->params['dry-run']) {
