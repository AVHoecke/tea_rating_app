...
/**
 * Content stack, used for nested templates that all *
 * @var array
 */
 ...
 */
	public function element($name, $data = array(), $options = array()) {
		...
		if (empty($options['ignoreMissing'])) {
			...
			trigger_error(__d('cake_dev', 'Element Not Found: {0}', $file), E_USER_NOTICE);
		}
	}
 ...
 */
	public function loadHelpers() {
		$helpers = HelperRegistry::normalizeObjectArray($this->helpers);
 ...
 * @throws \Exception when a block is left open.
 */
	protected function _render($viewFile, $data = array()) {
		...
		if ($initialBlocks !== $remainingBlocks) {
			throw new \Exception(__d('cake_dev', 'The "{0}" block was left open. Blocks are not allowed to cross files.', $this->Blocks->active()));
		}
