...
use lowercase_underscored function name)'));
 ...
 */
	protected function _loadController() {
		...
		if (!class_exists($controllerClassName)) {
			...
			$this->err(__d('cake_console', "The file '{0}' could not be found.\nIn order to bake a view, you'll need to first create the controller.", $file));
			return $this->_stop();
		}
 ...
 */
	public function customAction() {
		$action = '';
		while (!$action) {
			$action = $this->in(__d('cake_console', 'Action Name? (			if (!$action) {
		...
		$this->hr();
		$this->out(__d('cake_console', 'Controller Name: {0}', $this->controllerName));
		$this->out(__d('cake_console', 'Action Name:     {0}', $action));
		$this->out(__d('cake_console', 'Path:            {0}', $this->getPath() . $this->controllerName . DS . Inflector::underscore($action) . ".ctp"));
		$this->hr();
 ...
 */
	public function bake($action, $content = '') {
		...
		$this->out("\n" . __d('cake_console', 'Baking `{0}` view file...', $action), 1, Shell::QUIET);
		$path = $this->getPath();
