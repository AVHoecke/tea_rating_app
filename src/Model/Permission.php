 ...
 */
	public function check($aro, $aco, $action = '*') {
		...
		if (!$aroPath) {
			$this->log(__d('cake_dev', "{0} - Failed ARO node lookup in permissions check. Node references:\nAro: {1}\nAco: {2}",
					'DbAcl::check()',
					print_r($aro, true),
					print_r($aco, true)),
				E_USER_WARNING
		...
		if (!$acoPath) {
			$this->log(__d('cake_dev', "{0} - Failed ACO node lookup in permissions check. Node references:\nAro: {1}\nAco: {2}",
					'DbAcl::check()',
					print_r($aro, true),
					print_r($aco, true)),
				E_USER_WARNING
		...
		if ($action !== '*' && !in_array('_' . $action, $permKeys)) {
			$this->log(__d('cake_dev', "ACO permissions key {0} does not exist in {1}", $action, 'DbAcl::check()'), E_USER_NOTICE);
			return false;
		}
 ...
 */
	public function allow($aro, $aco, $actions = '*', $value = 1) {
		...
		if (!$perms) {
			$this->log(__d('cake_dev', '{0} - Invalid node', 'DbAcl::allow()'), E_USER_WARNING);
			return false;
		}
		if (isset($perms[0])) {
		...
		if ($actions === '*') {
			...
			foreach ($actions as $action) {
				...
				if (!in_array($action, $permKeys, true)) {
					throw new AclException(__d('cake_dev', 'Invalid permission key "{0}"', $action));
				}
				$save[$action] = $value;
			}
		}
