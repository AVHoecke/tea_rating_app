 ...
 */
abstract class BaseAuthenticate implements EventListener {
 ...
 */
	public function passwordHasher() {
		...
		if (!class_exists($className)) {
			throw new \Exception(__d('cake_dev', 'Password hasher class "{0}" was not found.', $class));
		}
		if (!is_subclass_of($className, 'AbstractPasswordHasher')) {
			throw new \Exception(__d('cake_dev', 'Password hasher must extend AbstractPasswordHasher class.'));
		}
