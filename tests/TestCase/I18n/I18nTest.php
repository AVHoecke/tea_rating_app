 ...
 */
	public function testFloatValue() {
		...
		$result = __n('{0} = 1', '%d = 0 or > 1', (float)1);
		$expected = '%d is 1 (translated)';
		$this->assertEquals($expected, $result);

		$result = __n('{0} = 1', '%d = 0 or > 1', (float)2);
		$expected = "%d ends in 2-4, not 12-14 (translated)";
		$this->assertEquals($expected, $result);

		$result = __n('{0} = 1', '%d = 0 or > 1', (float)5);
 ...
 */
	protected function _plural($upTo = 25) {
		...
		for ($number = 0; $number <= $upTo; $number++) {
			$plurals[] = sprintf(__n('{0} = 1', '%d = 0 or > 1', (float)$number), (float)$number);
		}
		return $plurals;
	}
 ...
 */
	protected function _pluralFromCore($upTo = 25) {
		$plurals = array();
		for ($number = 0; $number <= $upTo; $number++) {
			$plurals[] = sprintf(__n('{0} = 1 (from core)', '%d = 0 or > 1 (from core)', (float)$number), (float)$number);
		}
		return $plurals;
	}
}
