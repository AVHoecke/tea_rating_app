			...
			if (!empty($sessionConfig['ini']) && is_array($sessionConfig['ini'])) {
				foreach ($sessionConfig['ini'] as $setting => $value) {
					if (ini_set($setting, $value) === false) {
						throw new CakeSessionException(__d('cake_dev', 'Unable to configure the session, setting {0} failed.', $setting));
					}
				}
			}
		}
 ...
 */
	protected static function _getHandler($handler) {
		...
		/* TODO: App::uses($class, $plugin . 'Model/Datasource/Session'); */
		if (!class_exists($class)) {
			throw new CakeSessionException(__d('cake_dev', 'Could not load {0} to handle the session.', $class));
		}
		$handler = new $class();
		if ($handler instanceof SessionHandlerInterface) {
			return $handler;
		}
		throw new CakeSessionException(__d('cake_dev', 'Chosen SessionHandler does not implement SessionHandlerInterface it cannot be used with an engine key.'));
	}
