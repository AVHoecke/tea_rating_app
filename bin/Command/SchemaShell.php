 ...
 */
	public function view() {
		...
		$this->err(__d('cake_console', 'Schema file ({0}) could not be found.', $file));
		return $this->_stop();
	}
 ...
 */
	public function generate() {
		...
		if ($this->Schema->write($content)) {
			$this->out(__d('cake_console', 'Schema file: {0} generated', $content['file']));
			return $this->_stop();
		}
 ...
 */
	public function dump() {
		...
		$db = ConnectionManager::get($this->Schema->connection);
		...
		if ($write) {
			...
			if ($File->write($contents)) {
				$this->out(__d('cake_console', 'SQL dump file created in {0}', $File->pwd()));
				return $this->_stop();
			}
 ...
 */
	protected function _loadSchema() {
		...
		if (!$Schema) {
			...
			$this->err(__d('cake_console', '- file: {0}', $this->Schema->path . DS . $this->Schema->file));
			$this->err(__d('cake_console', '- name: {0}', $this->Schema->name));
			return $this->_stop(2);
		}
		$table = null;
 ...
 */
	protected function _run($contents, $event, CakeSchema $Schema) {
		...
		foreach ($contents as $table => $sql) {
			if (empty($sql)) {
				$this->out(__d('cake_console', '{0} is up to date.', $table));
			} else {
				if ($this->_dry === true) {
					$this->out(__d('cake_console', 'Dry run for {0} :', $table));
					$this->out($sql);
				} else {
					...
					if (!empty($error)) {
						...
						$this->out(__d('cake_console', '{0} updated.', $table));
					}
				}
			}
		}
