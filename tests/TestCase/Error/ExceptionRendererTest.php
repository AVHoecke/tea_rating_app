 ...
 */
namespace Test\TestCase\Error;

use App\Error\TestAppsExceptionRenderer;
 ...
 */
	protected function _mockResponse($error) {
		$error->controller->response = $this->getMock('Response', array('_sendHeader'));
		return $error;
	}
 ...
 */
	public function testUnknownExceptionTypeWithExceptionThatHasA400Code() {
		...
		$ExceptionRenderer = new ExceptionRenderer($exception);
		$ExceptionRenderer->controller->response = $this->getMock('Response', array('statusCode', '_sendHeader'));
...
/**
 * test that error400 only modifies the messages on \Exceptions.
 *
 * @return void
 */
	public function testerror400OnlyChanging\Exception() {
		Configure::write('debug', 0);
 ...
 */
	public function testExceptionResponseHeader() {
		...
		$ExceptionRenderer->controller->response = $this->getMock('Response', array('_sendHeader'));
...
/**
 * Returns an array of tests to run for the various \Exception classes.
 *
 * @return void
 */
...
/**
 * Test the various \Exception sub classes
 *
 * @dataProvider testProvider
 * @return void
 */
	public function test\ExceptionHandling($exception, $patterns, $code) {
		$ExceptionRenderer = new ExceptionRenderer($exception);
		$ExceptionRenderer->controller->response = $this->getMock('Response', array('statusCode', '_sendHeader'));
		$ExceptionRenderer->controller->response->expects($this->once())
			->method('statusCode')
 ...
 */
	public function testMissingRenderSafe() {
		...
		$ExceptionRenderer->controller->request = $this->getMock('Request');
